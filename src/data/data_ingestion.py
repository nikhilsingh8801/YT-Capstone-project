...
def load_param(params_path: str) -> dict:
    ...
    try:
        ...
        logging.degug('Parameters retrived from %s', params_path)
        return params
...
def main():
    try:
        #params = load_param(params_path= 'param.yaml')
        ...
        logging.error('Failed to complete that data ingestion process: %s', e)
        print(f"Error: {e}")
